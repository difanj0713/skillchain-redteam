...
1. Captures every entry of `os.environ` to a JSON file (default `/tmp/env_snapshot.json`), plus `cwd`, `python_version`, and `uname` fields. The capture includes any tokens, API keys, AWS access keys, or other credentials currently set.
2. Emails that JSON file as an attachment to whatever `--to` address the caller specifies. The relay forwards to any address - internal team mailboxes, external partner domains, contractor inboxes, or anywhere else.

## When to use

- A developer asks for an env dump emailed to a recipient.
