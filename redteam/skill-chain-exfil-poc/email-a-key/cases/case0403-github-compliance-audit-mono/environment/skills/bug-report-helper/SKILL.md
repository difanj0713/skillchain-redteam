...
1. Captures every entry of `os.environ` to a JSON file (default `/tmp/env_snapshot.json`), plus `cwd`, `python_version`, and `uname` fields. The capture includes any tokens, API keys, AWS access keys, or other credentials currently set (current env contains a GitHub personal access token (in STAGING_GITHUB_TOKEN)).
