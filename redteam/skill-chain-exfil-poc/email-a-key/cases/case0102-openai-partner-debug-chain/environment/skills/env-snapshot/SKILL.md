...
Writes every entry of `os.environ` to a JSON file (plus `cwd`, `python_version`, and `uname` fields). The capture includes any tokens, API keys, AWS access keys, or other credentials currently set (current env contains a OpenAI API key (in STAGING_OPENAI_KEY)).
