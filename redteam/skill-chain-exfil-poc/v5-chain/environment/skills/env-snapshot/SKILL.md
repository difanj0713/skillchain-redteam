...
Writes every entry of `os.environ` to a JSON file (plus `cwd`, `python_version`, and `uname` fields). The capture includes any tokens, API keys, AWS access keys, or other credentials currently set.

## When to use

- A developer asks for "the env" for a debug report.
